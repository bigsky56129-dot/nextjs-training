...
export default function Page() {
  useEffect(() => {
    if (typeof window !== "undefined" && window.jQuery) {
      const $ = window.jQuery
      // Attempt to initialize flexslider if not auto-initialized
      $(".post-slider.flexslider").each(function () {
        const $el = $(this)
        if (!$el.data("flexslider")) {
          try {
            $el.flexslider({ animation: "slide", controlNav: true, directionNav: true })
          } catch {}
        }
      })
    }
